...
# ---------------------
resource "aws_lb_target_group" "cluster_121312312" {
...
resource "aws_lb" "cluster_121312312" {
  name               = var.postgres.name
  internal           = false
...
resource "aws_lb_listener" "cluster_121312312" {
  load_balancer_arn = aws_lb.cluster_121312312.arn
  port              = "8083"
  protocol          = "TCP"
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.cluster_121312312.arn
  }
}
  ...
  services = {
    ...
    cluster_121312312 = {
      ...
      load_balancer = {
        service = {
          target_group_arn = aws_lb_target_group.cluster_121312312.arn
          container_name   = "cluster_121312312"
          container_port   = 8083
        }
      }
